...
class Admin_menu(ctk.CTkFrame):
    ...
    def load_user_data(self):
        ...
        try: 
            ...
            print(f"🔍 [admin_menu] user_id retrieved: {user_id}")  # Debug
    ...
    def edit_user(self):
        """ Function to edit user information (name, email, etc.) """
    ...
    def delete_user(self):
        ...
        user_id = self.conn.get_user_id()  # Get the user ID
    ...
    def edit_account_balance(self, account_id):
        ...
        self.edit_account_window.title("Modifier le solde du compte")
        self.edit_account_window.geometry("400x200")

        self.account_id_to_edit = account_id  # Sauvegarder l'ID du compte à modifier

        # Récupérer les informations du compte à partir de la base de données
        self.conn.connect_db()
        try:
            query = "SELECT name, balance FROM accounts WHERE id = %s"
            self.conn.cursor.execute(query, (account_id,))
            account_data = self.conn.cursor.fetchone()

            self.account_name_entry = ctk.CTkLabel(self.edit_account_window, text=f"Nom du compte: {account_data[0]}")
            self.account_name_entry.grid(row=0, column=1, padx=10, pady=10)

            self.balance_entry = ctk.CTkEntry(self.edit_account_window, placeholder_text="Nouveau solde", textvariable=ctk.DoubleVar(value=account_data[1]))
            self.balance_entry.grid(row=1, column=1, padx=10, pady=10)

            # Bouton pour sauvegarder les modifications
            self.save_balance_button = ctk.CTkButton(self.edit_account_window, text="Sauvegarder", command=self.save_account_balance)
            self.save_balance_button.grid(row=2, column=0, columnspan=2, pady=15)
